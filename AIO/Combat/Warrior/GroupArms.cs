...
namespace AIO.Combat.Warrior
{
    ...
    internal class GroupArms : BaseRotation
    {
        ...
        protected override List<RotationStep> Rotation => new List<RotationStep>
        {
            ...
            new RotationStep(new RotationSpell(SpellIds.Warrior.Charge), 1f, (s,t) => t.GetDistance > 8 && t.HasTarget && !t.IsTargetingMe && RotationFramework.PartyMembers.Any(m => m.Position.DistanceTo(t.Position) < 7) , RotationCombatUtil.BotTargetFast, forcedTimerMS: 1000),
            ...
            new RotationStep(new RotationSpell(SpellIds.Warrior.Overpower), 3.5f, RotationCombatUtil.Always, RotationCombatUtil.BotTargetFast),

            // Rage dumps
            new RotationStep(new RotationSpell(SpellIds.Warrior.Cleave), 4f, (s,t) => Me.CRage() > 50 && !_cleaveOn && !_heroicStrikeOn && _cleavableEnemies.Count > 0 && _nbEnemiesAroundMe >= Settings.Current.GroupArmsAoe, RotationCombatUtil.BotTargetFast, ignoreGCD: true),
            ...
            // AOE
            new RotationStep(new RotationSpell(SpellIds.Warrior.SweepingStrikes), 6f, (s,t) => _nbEnemiesAroundMe >= Settings.Current.GroupArmsAoe, RotationCombatUtil.BotTargetFast, ignoreGCD: true),
            new RotationStep(new RotationSpell(SpellIds.Warrior.Rend), 7f, (s,t) => _nbEnemiesAroundMe >= Settings.Current.GroupArmsAoe && !t.CHaveMyBuff(SpellIds.Warrior.Rend), RotationCombatUtil.BotTargetFast),
            new RotationStep(new RotationSpell(SpellIds.Warrior.Bladestorm), 8f, (s,t) => _nbEnemiesAroundMe >= Settings.Current.GroupArmsAoe, RotationCombatUtil.BotTargetFast),
            new RotationStep(new RotationSpell(SpellIds.Warrior.Rend), 8.5f, (s,t) => Settings.Current.GroupArmsSpreadRend, p => _enemiesAroundWithoutMyRend.FirstOrDefault()),
            ...
            new RotationStep(new RotationSpell(SpellIds.Warrior.EnragedRegeneration), 10f, (s,t) => Me.CHealthPercent() < Settings.Current.GroupArmsEnragedRegen, RotationCombatUtil.BotTargetFast),
            ...
            new RotationStep(new RotationSpell(SpellIds.Warrior.DemoralizingShout), 12f, (s,t) => Settings.Current.GroupArmsDemoShout && !t.CHaveBuff(SpellIds.Warrior.DemoralizingShout), RotationCombatUtil.BotTargetFast),
            new RotationStep(new RotationSpell(SpellIds.Warrior.IntimidatingShout), 13f, (s,t) => Settings.Current.GroupArmsIntimShout && _nbEnemiesAroundMeCasting > 0, RotationCombatUtil.BotTargetFast),
            ...
            new RotationStep(new RotationSpell(SpellIds.Warrior.Slam), 19f, (s,t) => Settings.Current.GroupArmsSlam && Me.CRage() > 20, RotationCombatUtil.BotTargetFast),
